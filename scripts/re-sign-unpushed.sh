#!/bin/sh
# POSIX layout is canonical upstream (repo-foundation). brew style runs the
# ShellCheck tool with --shell=bash --enable=all regardless of shebang; its
# --fix mode auto-applies the [[ ]] conversion -- so the optional style
# checks that conflict with POSIX form are exempted: SC2249
# (add-default-case), SC2250 (require-variable-braces), SC2292
# (require-double-brackets). SC2310/SC2311/SC2312 (set -e informational
# checks) are disabled to match Homebrew's own .shellcheckrc, which babble
# tracks verbatim. Numeric codes: names are not valid in disable=.
...
resign_one() {
  ...
  then
    ...
    for c in $(git -C "$repo_dir" rev-list --reverse --first-parent "$range_spec")
    do
