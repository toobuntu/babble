...
#
# Self-contained test harness for re-sign-unpushed.sh (throwaway repos under
...
SCRIPT=${1:-"$(cd "$(dirname "$0")" && pwd)/re-sign-unpushed.sh"}
[ -f "$SCRIPT" ] || {
...
no_n_in() { # no_n_in <repo> <range> <name>
  if git -C "$1" log --format='%G?' "$2" | grep -qx N
  then
