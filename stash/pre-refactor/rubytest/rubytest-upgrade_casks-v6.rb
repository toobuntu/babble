...
def token_installed?(token)
  # system("brew list | grep --quiet --fixed-strings --ignore-case #{token}") || system("mas list | grep --quiet --fixed-strings --ignore-case #{token}")
  begin
    stdout, _, status = Open3.capture3("brew list")
    raise "brew list failed" unless status.success?

    if stdout.include?(token.to_s)
      $stderr.puts "#{token} is installed."
    else
      $stderr.puts "#{token} is not installed."
    end
  rescue => e
...
def validate_config(config)
  ...
  config.each do |token, data|
    ...
    if token_installed?(token)
      data["bundle_ids"].each do |bundle_id|
        unless valid_bundle_id?(bundle_id)
...
def quit_app(bundle_id, config_entry)
  ...
  handle_quit_result(stdout, stderr, bundle_id)
end

def handle_quit_result(stdout, stderr, bundle_id)
  # Debug output
  puts "stdout: #{stdout}"
  puts "stderr: #{stderr}"
