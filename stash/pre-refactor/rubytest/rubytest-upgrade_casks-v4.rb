...
def sort_config(config)
  sorted_config = config.sort.to_h
  sorted_config.each do |key, value|
    if value.is_a?(Hash)
      sorted_config[key] = sort_config(value)
    elsif value.is_a?(Array) && value.all? { |item| item.is_a?(String) }
      sorted_config[key] = value.sort
    end
  end
  sorted_config
end

def config_needs_sorting?(config)
  sorted_config = sort_config(config)
  config != sorted_config
end

def reorganize_config_file(file_path)
  current_config = YAML.load_file(file_path)

  if config_needs_sorting?(current_config)
    sorted_config = sort_config(current_config)
