...
def self.validate_config(raw_config)
  ...
  homebrew_entries.each do |entry|
    ...
    if entry.key?("quit_message")
      existing_message = valid_entry["quit_message"]
      new_message = entry["quit_message"].to_s
  ...
  mas_entries.each do |entry|
    ...
    if bundle_ids.is_a?(Array)
      valid_bundle_ids = bundle_ids.select { |id| valid_bundle_id?(id) }
      invalid_bundle_ids = bundle_ids - valid_bundle_ids
      validation_errors.concat(invalid_bundle_ids.map { |id| "Invalid Bundle ID for MAS app #{app_id}: #{id} - #{entry["name"]}" })
...
begin
  ...
  BrewUpgrade.reorganize_config_file(BrewUpgrade::CONFIG_FILE)

  # Load and validate configuration
  config, conflicts, validation_errors, structural_issues =
  BrewUpgrade.load_and_validate_configuration(BrewUpgrade::CONFIG_FILE)
  ...
  outdated_cask_tokens = BrewUpgrade.outdated_casks_json
